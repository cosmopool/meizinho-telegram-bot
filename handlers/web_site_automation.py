...
def wait_download_complete(response_header: dict) -> str:
...
def wait_for_element_to_load(driver, element: tuple, seconds = 5) -> None:
    """Wait for an especific html element load"""

    WebDriverWait(driver, seconds).until(EC.presence_of_element_located(element))


def wait_for_element_and_click(driver, element: tuple, seconds = 5) -> None:
...
def get_document(driver, cnpj: str, date: str) -> str:
    ...
    wait_for_element_and_click(driver, (By.ID, 'continuar'))
    log.info(f"Sign in with: {cnpj}")
    
    wait_for_element_and_click(driver, (By.CLASS_NAME, 'glyphicon.glyphicon-check'))
    
    wait_for_element_and_click(driver, (By.CLASS_NAME, 'form-control'))
    
    log.info(f"Selecting given year")
    wait_for_element_and_click(driver, (By.LINK_TEXT, date))
    
    wait_for_element_and_click(driver, (By.CLASS_NAME, "btn-success"))
    
    wait_for_element_and_click(driver, (By.CLASS_NAME, "form-inline"))
    ...
    for month in list_of_months:
        try:
            ...
            wait_for_element_and_click(driver, (By.ID, "btnEmitirDas"))
    
    if found_document_to_download:
        wait_for_element_and_click(driver, (By.CLASS_NAME, "btn.btn-success"))
        path_to_file = wait_download_complete(driver.requests[-1].response.headers)
    else:
